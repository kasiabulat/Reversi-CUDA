...
const collection Board::DIRECTIONS = collection({
	make_pair(-1,-1), //NW
	make_pair(-1,0), //N
	make_pair(-1,1), //NE
	make_pair(0,-1), //W
...
pair<int,int> Board::get_cell_coordinates(int cell) {
	return make_pair(cell/8,cell%8);
}
...
bool Board::is_players_piece_on_the_end(int cell, pair<int,int> direction) {
	auto current_cell = cell;
	auto direction_row = direction.first;
	auto direction_column = direction.second;
